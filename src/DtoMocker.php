...
class DtoMocker
{
     ...
     * @return mixed
     */
    protected function generateForType(?ReflectionType $type, int $maxDepth)
    {
        if ($type === null) {
        ...
        if ($type->isBuiltin()) {
            switch ($name) {
                case 'int':
                    return ($this->generators['int'])();
                case 'float':
                    return ($this->generators['float'])();
                case 'bool':
                    return ($this->generators['bool'])();
                case 'array':
                    return ($this->generators['array'])();
                case 'string':
                    return ($this->generators['string'])();
                default:
